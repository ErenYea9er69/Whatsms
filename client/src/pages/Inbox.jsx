    ...
    return (
        <div className="inbox-page p-6 h-full flex flex-col bg-gray-50 dark:bg-black transition-colors duration-300">
            <header className="mb-6 flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{t('inbox_title')}</h1>
            ...
            <div className="bg-white dark:bg-[#09090b] rounded-lg shadow overflow-hidden flex-1 border border-gray-100 dark:border-zinc-800">
                <div className="grid grid-cols-1 divide-y divide-gray-100 dark:divide-zinc-800">
                    {loading ? (
                        <div className="p-4 text-center text-gray-500 dark:text-zinc-400">{t('loading_conversations')}</div>
                    ) : conversations.length === 0 ? (
                        <div className="p-4 text-center text-gray-500 dark:text-zinc-400">{t('no_conversations')}</div>
                    ) : (
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-medium text-gray-900 dark:text-zinc-100">{conv.contact?.name || t('unknown_contact')}</h3>
                                            <p className="text-sm text-gray-500 dark:text-zinc-400 truncate max-w-md">
                                                {conv.lastMessage || t('no_messages_yet')}
                                            </p>
                                        </div>
                                    </div>
